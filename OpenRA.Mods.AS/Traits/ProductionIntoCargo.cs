...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProductionIntoCargoInfo : PausableConditionalTraitInfo
	{
		[FieldLoader.Require]
		[Desc("e.g. Infantry, Vehicles, Aircraft, Buildings")]
		public readonly string[] Produces = { };

		public override object Create(ActorInitializer init) { return new ProductionIntoCargo(init, this); }
	}

	public class ProductionIntoCargo : PausableConditionalTrait<ProductionIntoCargoInfo>
	{
		RallyPoint rp;

		public string Faction { get; private set; }

		public ProductionIntoCargo(ActorInitializer init, ProductionIntoCargoInfo info)
			: base(info)
		{
		...
		public virtual void DoProductionIntoCargo(Actor self, ActorInfo producee, ExitInfo exitinfo, string productionType, TypeDictionary inits)
		{
			var exit = CPos.Zero;
		...
		public virtual void DoProductionIntoCargo(Actor self, ActorInfo producee, ExitInfo exitinfo, string productionType, TypeDictionary inits, ManipulateActorCallback OnSpawnCallback)
		{
			var exit = CPos.Zero;
			var exitLocations = new List<CPos>();
		...
		public virtual bool Produce(Actor self, ActorInfo producee, string productionType, TypeDictionary inits, int refundableValue)
		{
			...
			{
				DoProductionIntoCargo(self, producee, exit?.Info, productionType, inits);
				return true;
			}
		...
		public virtual bool Produce(Actor self, ActorInfo producee, string productionType, TypeDictionary inits, int refundableValue, ManipulateActorCallback OnSpawnCallback)
		{
			...
			if (exit != null || self.OccupiesSpace == null || !producee.HasTraitInfo<IOccupySpaceInfo>())
			{
				DoProductionIntoCargo(self, producee, exit?.Info, productionType, inits, OnSpawnCallback);
				return true;
			}
